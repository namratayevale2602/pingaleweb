...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white shadow-lg sticky top-0 z-50">
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 max-h-[80vh] overflow-y-auto animate-slideDown">
          <div className="px-4 py-6 space-y-4">
            {navItems.map((item, index) => (
              <div key={index} className="space-y-2">
                {item.dropdown ? (
                  <>
                    <div className="font-semibold text-[#074a6b] text-lg border-b border-gray-200 pb-1">
                      {item.title}
                    </div>
                          ...
                          <ul className="space-y-2 pl-2">
                            {category.items.map((subItem, subIndex) => (
                              <li key={subIndex}>
                                <Link
                                  href={`/${subItem.slug}`}
                                  className="text-gray-600 hover:text-[#2ba5ea] block py-1 transition-colors"
                  ...
                  <Link
                    href={item.link}
                    className="font-semibold text-[#074a6b] text-lg block hover:text-[#2ba5ea] py-1 border-b border-gray-200"
                    onClick={() => setIsOpen(false)}
                  >
                    {item.title}
                  </Link>
